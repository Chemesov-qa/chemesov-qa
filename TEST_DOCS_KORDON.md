...
*   **Уникальный ID и атомарность:** Каждый кейс имеет идентификатор (например, `TC-MON-01`). Проверки атомарны, но группируются в рамках одной сессии, если завязаны на общий триггер (проезд ТС, обрыв связи).
*   **Детерминированные предусловия:** Состояние системы и «железа» задано конкретно: *«Статус комплекса: Online»*, *«Утилита fsutil доступна»*. Никакой двусмысленности на старте.
*   **Паспортизированные тестовые данные:** IP-адреса (`192.168.1.101`), учетные данные, пути к архивам (`D:\Archive`), пороговые значения (`90%`) — всё зафиксировано в кейсе.
*   **Измеримые критерии приёмки:** Ожидаемый результат — бинарный проверяемый факт. *«Задержка ≤ 5 сек»*, *«индикатор подсвечен красным»*, *«в логе запись без ошибок»* вместо абстрактного «работает корректно».
