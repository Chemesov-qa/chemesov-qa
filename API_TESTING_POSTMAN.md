...
*   **Проверка цепочки действий:** Мы убеждаемся, что данные правильно передаются между запросами<br>(например, созданное бронирование реально можно найти и изменить).
*   **Имитация реального пользователя:** Тесты проходят путь клиента от начала до конца, проверяя,<br>не «сломалось» ли что-то в середине процесса.
*   **Быстрый поиск ошибок:** Если после правок в коде разработчика что-то пойдет не так,<br>мы мгновенно увидим «красную» ошибку в отчете.
*   **Проверка на "дурака":** Специальные негативные тесты проверяют, что система не пустит пользователя<br>без пароля или с неверными данными.
