...
Запускать каждый запрос вручную — долго. Поэтому я объединил их в единый цикл автотестов. Это нужно для того, чтобы за 5 секунд проверить всю логику приложения: от создания брони до её удаления.

**Зачем это делается:**

*   **Проверка цепочки действий:** Мы убеждаемся, что данные правильно передаются между запросами (например, созданное бронирование реально можно найти и изменить).
*   **Имитация реального пользователя:** Тесты проходят путь клиента от начала до конца, проверяя, не «сломалось» ли что-то в середине процесса.
*   **Быстрый поиск ошибок:** Если после правок в коде разработчика что-то пойдет не так, мы мгновенно увидим «красную» ошибку в отчете.
*   **Проверка на "дурака":** Специальные негативные тесты проверяют, что система не пустит пользователя без пароля или с неверными данными.
